    ...
    <Card>
      <CardHeader className="pb-2">
        ...
        <p className="text-xs text-muted-foreground">
          Effect of each factor on this patient's 12-month numbers, banded low / medium / high
          (arrow shows the direction). Where a factor still has room to move, the green line
          underneath says what changing it is worth.
        </p>
      </CardHeader>
          ...
          {rows.map((r) => (
            <div key={r.label} className="contents">
              <div className="min-w-0">
                ...
                {(r.potGain > 0 || r.contGain > 0) && (
                  <span className="block text-[11px] leading-snug text-emerald-600 dark:text-emerald-400">
                    → {r.target}: {[
                      r.potGain > 0 ? `+${r.potGain} pp potency` : null,
                      r.contGain > 0 ? `+${r.contGain} pp continence` : null,
                    ]
                      .filter(Boolean)
                      .join(", ")}
                  </span>
